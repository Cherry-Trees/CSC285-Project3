...
private:
  ...
  */
  ...
  // of vertices (and their cost) that are adjacent to the indexed
...
public:
  ...
  // default contructor. Resulting Graph would have no vertices (and no edges)
  Graph(): _verts(), _adj() {}
  ...
  // returns a set of the vertices (names) in the graph
  std::set<std::string> getVertices() const;

  // for any one vertex, this returns a set of all other ajacent vertices
  std::set<std::string> getAdjacencies(const std::string &vertex) const;

  // for the associated two vertices, returns the cost of an edge between them
  //   * returns INT_MAX if there is no such edge (or vertices)
  ...
  // adds a single edge between the two specifed vertices (strings)
