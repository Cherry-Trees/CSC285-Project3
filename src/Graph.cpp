...
Graph::addVertex(const std::string &name)
{
  ...
  return _verts.insert(name).second;
}
...
{
  // ass both vertices (endpoints) of edge to graph
  addVertex(src);  
  addVertex(dest);
...
// get a set of the names of vertices adjacent to the specifed vertex
// (vertName)
std::set<std::string> 
Graph::getAdjacencies(const string &vertName) const
{
  set<string> ans; // initially, we have not found any such vertices. 
